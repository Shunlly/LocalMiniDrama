...
/**
 * 帧提示词装配：角色锚点、分镜上下文、画风合并与布局重生成用户提示。
 * 路由仍通过 framePromptService 调用，本模块不改变公开 API。
 */

const promptI18n = require('./promptI18n');
const angleService = require('./angleService');
const { parseNamesFromAnchorLines } = require('../utils/framePromptSanitize');
const { resolveStoryboardScope, loadDramaStyleRowForStoryboard } = require('./framePromptScope');
...
function mapCharacterAnchorRows(rows) {
  return (rows || []).map((r) => {
    let anchors = null;
...
function loadCharactersByIdsForDrama(db, ids, dramaId) {
  if (!ids.length || !Number.isInteger(dramaId) || dramaId <= 0) return [];
  ...
  if (!rows || rows.length === 0) {
    rows = db.prepare(
      `SELECT id, name, appearance, identity_anchors FROM character_libraries
        WHERE id IN (${placeholders}) AND deleted_at IS NULL
          AND (drama_id IS NULL OR drama_id = ?)`
    ).all(...ids, dramaId);
  }
  return rows || [];
}
...
function loadStoryboardCharacterNames(db, storyboardId) {
  ...
  const dramaId = scope ? scope.dramaId : null;
  let ids = [];
  ...
  if (!ids.length) {
    ...
    try {
      const sbRow = db.prepare('SELECT characters FROM storyboards WHERE id = ? AND deleted_at IS NULL').get(sid);
      if (sbRow?.characters && Number.isInteger(dramaId) && dramaId > 0) {
        ...
        if (nameMatches.length) {
          ...
          const nameRows = db.prepare(
            `SELECT id, name, appearance, identity_anchors FROM characters
              WHERE name IN (${namePlaceholders}) AND deleted_at IS NULL AND drama_id = ?`
          ).all(...nameMatches, dramaId);
          if (nameRows.length) {
            return mapCharacterAnchorRows(nameRows);
          }
        }
      }
    } catch (_) {}
  ...
  return mapCharacterAnchorRows(loadCharactersByIdsForDrama(db, ids, dramaId));
}
...
违背硬锁 = 生成失败；演化幅度过小导致运镜失效也属于不理想结果。`);
  ...
  if (sb.angle || (sb.angle_h && sb.angle_v && sb.angle_s)) {
    const isEnAngle = promptI18n.isEnglish(cfg);
    const angleDesc = expandAngleDescription(sb.angle, isEnAngle, sb.angle_h, sb.angle_v, sb.angle_s);
...
function applyDramaStyleToConfig(db, cfg, storyboardId) {
  const dramaRow = loadDramaStyleRowForStoryboard(db, storyboardId);
  if (!dramaRow) return cfg;
  ...
  return mergeCfgStyleWithDrama(next, dramaRow);
}

function buildLayoutRegenerationUserPrompt(sb, characterNames, prevSb, nextSb) {
  const sid = sb.id;
  const userLines = [
...
function normalizeGeneratedLayoutDescription(raw) {
  let newLayout = String(raw || '').trim()
    .replace(/^```[a-z]*\s*/i, '')
    .replace(/\s*```$/, '')
    .replace(/^["'“”‘’]+|["'“”‘’]+$/g, '')
    .trim();
...
module.exports = {
  ...
  expandAngleDescriptionLegacy,
  loadStoryboard,
  cleanAppearanceForIdentity,
