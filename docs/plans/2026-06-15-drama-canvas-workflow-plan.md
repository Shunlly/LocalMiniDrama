...
> 目标：在 LocalMiniDrama 现有 `project.json` / SQLite 数据之上，提供节点化画布工作台；列表模式（FilmCreate）与画布模式双视图、单数据源。仅吸收通用工作流思想，不复制参考项目的 AGPL 源码或资产。

**最后更新**：2026-07-27（阶段 E：制作 + 自由双模式创作层）
...
6. **自由层隔离**：自由节点和连线写入 `drama.metadata.free_canvas`；生产图继续使用原数据模型，保存时保留未知 metadata 字段。
...
## 阶段 E：自由创作层（2026-07-27，实现完成；最终运行门禁待验证）

自由画布不是新路由，也不替换生产流水线。`/film/:id/canvas` 在同一个桌面工作台中提供「制作」和「自由」两种模式：制作模式继续呈现生产图，自由模式叠加可编辑的创作节点，并通过明确动作转换为生产引用。

### 已交付

- [x] 同一路由「制作 / 自由」模式切换，切换和保存不会删除或改写生产图
- [x] `text`、`image`、`video`、`config`、`reference` 五类自由节点，以及自由节点连线
- [x] `metadata.free_canvas` 持久化；部分更新保留 `canvas_layout`、`workflow_groups` 与未知 metadata
- [x] 单选、多选、框选、连线、复制粘贴、删除、撤销和重做
- [x] 素材搜索、图片/视频筛选、分组折叠、上传入口和拖入画布
- [x] 持久显示净化后的保存失败原因，并对失败变更执行精确重试
- [x] 符合资格的本地图片/视频保存为素材；不合格节点显示具体原因
- [x] 通过显式目标选择把自由内容转换为生产引用；自由节点保留，失败可重试
- [x] 项目 ZIP 导入/导出保留并安全重映射自由画布，专项复审 `Spec PASS / Security PASS`
- [x] 8 项产品验收完成三轮复审，结论为 `Spec PASS / Quality PASS`
- [x] 浏览器 E2E 代码与证据契约完成复审，结论为 `Spec PASS / Quality PASS`

### 发布前仍待完成

- [ ] 在最终候选源码上重建 Docker，并执行真实生产 E2E
- [ ] 完成 1280x720、1366x768、1440x900 的亮色/暗色六组运行矩阵并人工检查证据
- [ ] 在同一源码修订上完成其余发布门禁；在此之前自由画布最终状态为 `UNVERIFIED`

### 延期范围

- [ ] 移动端重排、触控手势与移动画布/列表降级
- [ ] 新的真实 Provider 路由与第三方账号/模型深度联调
- [ ] 多人协作、冲突合并与在线共享
- [ ] 完整 Agent/MCP 对话、工具编排与自动执行面板

> 自动化测试只使用本地协议兼容测试服务，不调用外部真实 Provider。
