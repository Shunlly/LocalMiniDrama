...
- **逐制品 Fuse 证明**：Setup、Portable、Unpacked 必须各解析出且仅解析出一个 Electron 应用，逐应用路径和 Fuse 状态写入最终安全证据，并由离线验证器重新检查。
- **扫描输入可追溯**：安全证据同时记录 Trivy 漏洞库 schema、更新时间和配置规则包 digest，保留发布时实际使用的扫描输入身份。
- **素材选择器并发安全**：全局素材选择器改为最新请求优先并取消旧请求，旧成功、旧失败和旧 Toast 不再覆盖或干扰新查询；加载、错误、空态、底部状态和确认动作保持一致，关闭后会失效未完成请求并重置筛选。
- **素材失败与辨识体验**：底层 `Network Error` 不再直接暴露给用户，失败态统一显示安全中文说明和原位重试；截断的长素材名支持悬停查看全文。
- **素材空态与返回体验**：分镜素材选择器空态可清除筛选或直接前往素材中心上传，安全保留原制作台返回地址；完整名称与辅助说明显式关联，项目导入提示关闭后焦点回到导入命令。
- **项目导入失败恢复**：非法扩展名和后端导入失败会在项目首页持续显示安全文件名、清洗后的原因、重新选择入口和关闭操作，不再只依赖短暂消息。
- **中文项目媒体播放与主动内容隔离**：静态媒体响应使用安全媒体 MIME 白名单，不再把含中文的绝对路径误写入 `Content-Type`；HTML、JavaScript、SVG、XML 和未知扩展统一下载且启用 `nosniff`，中文项目视频的 Range、`206` 和 `video/mp4` 进入回归测试。
...
- **生产容器加固与镜像扫描**：后端、前端和本地 E2E Provider 启用只读根文件系统、`no-new-privileges`、能力裁剪与受限临时目录；CI 对最终镜像 ID 执行固定 digest 的 Trivy High / Critical 扫描，发布标签必须来自 `main`。
- **ZIP 依赖安全升级**：后端与桌面将 `adm-zip` 固定到 0.6.0，关闭恶意 ZIP 伪造 4GB 分配漏洞，并增加导入回归和三包官方依赖审计。
- **中文路径完整播放证据**：生产 E2E 使用含中文的项目名，两个桌面视口的成片与分镜视频必须真实播放到 `ended`，同时记录 Unicode 路径证据。

### 发布

- `v1.3.2` 标签保留为未产出 Release 附件的失败发布尝试；正式 Windows 桌面、Web 与 Docker 发布由 `v1.3.3` 取代。失败标签不移动、不重写。
...
- **Windows Docker 启动兼容性**：Shell entrypoint 强制以 LF 检出，镜像构建时再次归一化换行，并在 Docker 制品验证中检查 shebang 与 CRLF，避免 Windows 工作树生成无法启动的容器。
- **Windows 产品验收测试兼容性**：读取路由源码前统一换行符，产品经理验收测试不再依赖 LF 空行，正式 Release 会在 Windows runner 上完整执行前端 224 项测试。

### 发布

- `v1.3.1` 标签保留为未产出 Release 附件的失败发布尝试；`v1.3.2` 作为后续候选继续验证，但最终也未产出附件，后续处理见 `v1.3.3`。失败标签不移动、不重写。
...
- **Windows 发布测试兼容性**：前端与后端测试脚本改用 Node.js 测试运行器的原生自动发现，不再依赖 Windows shell 不支持的 `test/*.test.js` 通配符展开。
- **发布契约回归保护**：根发布契约锁定前后端测试命令为跨平台写法，后续改动若重新引入 shell 通配符会在 CI 提前失败。

### 发布

- `v1.3.0` 标签保留为未产出 Release 制品的失败发布尝试；`v1.3.1` 作为后续候选继续验证，失败标签不做移动或重写。
...
- **桌面 Web 流程收口**：项目详情增加就绪度与唯一下一步；故事素材改为导入、处理、QA、修复、剧集/时间线五步流程。
- **生成动作反馈**：统一禁用按钮视觉，并为制作页和画布的关键动作补充不可用原因与可执行空状态。
- **AI 配置**：增加文本、素材图片、分镜图片、视频、TTS 五类服务覆盖摘要；配置弹窗按基础、高级接口、模型和策略渐进展开。
- **制作与画布工具**：制作页全流程设置收进独立面板，画布工具按创建内容、工作流、批量生成分组，工作流操作按选择上下文显示。
- **首页与素材中心**：修复项目卡片标题/操作重叠，统一素材中心入口、上传、URL 导入说明和空状态行动。
- **组件维护性**：从 `FilmCreate.vue` 抽离流水线控制面板与动作门禁，画布拆分桌面工具栏、工作流工具和空状态组件。

### 安全

- **素材上传加固**：素材中心上传改用临时磁盘文件并增加并发与磁盘预留门禁；图片由 `sharp` 完整解码，视频由 `ffprobe` 验证容器和视频流，再分配服务端扩展名。验证、数据库写入或响应失败都会清理临时/回滚文件。
- **素材文件生命周期**：删除上传来源素材时，仅删除受控 `uploads` 目录内且没有其他有效素材引用的实际文件。
