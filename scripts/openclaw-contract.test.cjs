...
const taskAssembly = read('backend-node', 'src', 'services', 'taskAssembly.js')
...
const dramaServiceWrites = read('backend-node', 'src', 'services', 'dramaServiceWrites.js')
const dramaAssembly = read('backend-node', 'src', 'services', 'dramaAssembly.js')
...
const storyboardProcess = read('backend-node', 'src', 'services', 'episodeStoryboardProcess.js')
const imageService = read('backend-node', 'src', 'services', 'imageService.js')
const imageServiceQuery = read('backend-node', 'src', 'services', 'imageServiceQuery.js')
...
test('backend sources expose the authoritative OpenClaw field contract', () => {
  ...
  assert.match(characterGeneration, /Number\(episode\.drama_id\) !== Number\(req\.drama_id\)/)
  assert.match(characterGeneration, /new Error\('剧集不属于当前项目'\)/)
  assert.match(characterGeneration, /error\.code = 'BAD_REQUEST'/)
  assert.doesNotMatch(characterGeneration, /episode_id must belong to drama_id/)
  assert.doesNotMatch(characterGeneration, /req\.count/)

  assert.match(promptOverrides, /const \{ content \} = req\.body \|\| \{\}/)
  assert.match(taskAssembly, /id: row\.id[\s\S]*result: row\.result/)
  assert.match(taskService, /rowToTask/)
  ...
  assert.match(dramaServiceWrites, /return getDramaById\(db, id\)/)
  assert.match(dramaService, /createDrama/)
  assert.match(dramaAssembly, /function rowToDrama[\s\S]*id: r\.id/)
  assert.match(responseHelpers, /function created\(res, data\)[\s\S]*\{ success: true, data \}/)
  assert.match(storyboardProcess, /meta\.video_clip_duration/)
  assert.match(storyboardService, /generateStoryboard/)
  assert.match(imageServiceQuery, /JOIN episodes e ON e\.id = s\.episode_id AND e\.deleted_at IS NULL/)
  assert.match(imageServiceQuery, /Number\(scope\.drama_id\) !== dramaId/)
  assert.match(imageServiceQuery, /imageBadRequest\('分镜不属于当前项目'\)/)
  assert.match(imageService, /resolveImageGenerationScope\(db, req\)/)
  assert.doesNotMatch(imageServiceQuery, /storyboard_id must belong to drama_id/)
})
