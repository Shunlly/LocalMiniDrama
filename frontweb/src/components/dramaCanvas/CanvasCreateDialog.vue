<template>
  ...
  >
    ...
    <template #footer>
      <el-button :aria-label="`取消${dialogTitle}`" @click="visible = false">取消</el-button>
      <el-button type="primary" :loading="submitting" :aria-label="submitting ? `正在${dialogTitle}` : `确定${dialogTitle}`" @click="onSubmit">创建</el-button>
    </template>
  </AccessibleDialog>
</template>
