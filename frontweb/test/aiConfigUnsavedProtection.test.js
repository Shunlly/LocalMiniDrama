...
test('production AI config dialog uses the same guard for chrome and custom back controls', () => {
  ...
  assert.match(filmCreateSource, /await aiConfigContentRef\.value\?\.requestClose\?\.\(\)/)
  assert.match(filmCreateSource, /const hasUnsavedAiConfig = showAiConfigDialog\.value\s*&& aiConfigContentRef\.value\?\.hasUnsavedChanges\?\.\(\)/)
  assert.match(filmCreateSource, /async function allowNavigationAfterDraftFlush\(\) \{[\s\S]*requestAiConfigWorkspaceNavigation\(\)/)
})
