...
test('卡片下一步文案与跳转一致：无有效剧集去创建，有剧集才继续制作', async () => {
  ...
  assert.equal(projectCardOpenLabel(blank, false), '打开项目「空白本」')
  assert.equal(projectCardOpenLabel(ready, false), '打开项目「雨巷」')
  assert.equal(projectCardOpenLabel({ title: '' }, true), '打开项目「未命名项目」')
