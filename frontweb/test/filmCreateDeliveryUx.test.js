...
test('没有可播放分镜视频时给出中文空状态和下一步入口', () => {
  ...
  assert.equal(noStoryboards.guidanceAnchor, 'anchor-storyboard')
  ...
  assert.equal(missingVideos.guidanceAnchor, 'anchor-storyboard-images')
  ...
  assert.doesNotMatch(deliveryPanelCopy, /guidanceHref/)
  assert.doesNotMatch(deliveryPanelVue, /guidanceHref/)
  assert.match(deliveryPanelVue, /<button[\s\S]*data-testid="delivery-empty-action"/)
  assert.match(deliveryPanelVue, /@click="\$emit\('scroll-to-anchor', panelState\.guidanceAnchor, panelState\.guidanceAnchor\)"/)
  assert.doesNotMatch(deliveryPanelVue, /href="#anchor/)
})
...
test('未选剧集时禁用原因可见，不会误导去生成分镜视频', () => {
  ...
  assert.equal(missingEpisode.guidanceAnchor, '')
