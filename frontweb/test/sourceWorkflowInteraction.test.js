...
test('来源工作流失败不再直出 e.message，取消静默，QA 问题过滤英文和密钥', () => {
  ...
  assert.match(source, /toUserFacingError\(e, '质量检查失败'\)/)
  ...
  assert.doesNotMatch(source, /e\.message \|\| '质量检查失败'/)
...
test('来源工作流失败文案保留结构化中文，过滤英文异常和密钥', () => {
  ...
  assert.equal(toUserFacingError({ message: 'Failed to fetch' }, '质量检查失败'), '质量检查失败')
