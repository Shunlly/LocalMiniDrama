...
test('FilmCreate 把视频配置交给独立面板并保留成片选项', () => {
  ...
  assert.match(filmCreateSource, /:compose-action-disabled-reason="composeActionDisabledReason"/)
  assert.match(filmCreateSource, /:video-status="videoStatus"/)
})
...
test('FilmCreate 把资源管理交给独立面板并保留折叠与空状态', () => {
  ...
  assert.match(filmCreateSource, /resourcePanel: \{[\s\S]*propItems: props[\s\S]*onGenerateCharacters/)
...
test('FilmCreate 把分镜生成交给独立面板并保留锚点', () => {
  ...
  assert.match(filmCreateSource, /storyboardPanel: \{[\s\S]*onGenerateStoryboard/)
...
test('FilmCreate 把模板头交给独立组件并保留当前集与主题入口', () => {
  ...
  assert.match(filmCreateSource, /:project-page-title="projectPageTitle"/)
  assert.match(filmCreateSource, /:selected-episode-id="selectedEpisodeId"/)
  assert.match(filmCreateSource, /@go-list="goList"/)
  assert.match(filmCreateSource, /@episode-select="onEpisodeSelect"/)
  assert.match(filmCreateSource, /@go-canvas-mode="goCanvasMode"/)
  assert.match(filmCreateSource, /@toggle-theme="toggleTheme"/)
  assert.match(filmCreateSource, /@open-ai-config="openAiConfig"/)
