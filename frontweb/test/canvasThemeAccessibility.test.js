...
const canvasCssSource = read('../src/views/DramaCanvas.css')
...
const windowedListSource = read('../src/components/dramaCanvas/CanvasWindowedList.vue')
const referenceBarSource = read('../src/components/dramaCanvas/CanvasStoryboardPanelReferences.vue')
const freeToolbarSource = read('../src/components/dramaCanvas/FreeCanvasToolbar.vue')
const workspaceSource = read('../src/components/dramaCanvas/CanvasWorkspace.vue')
...
test('769px 展开侧栏和常规桌面窄屏时画布工具条/节点列表/参考图条换行而不裁掉按钮', () => {
  assert.match(canvasCssSource, /\.drama-canvas-page \{[\s\S]*?max-width: 100%;[\s\S]*?min-width: 0;/)
  assert.match(canvasCssSource, /\.canvas-sidebar \{[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;/)
  assert.match(canvasCssSource, /:deep\(\.free-canvas-bottom-toolbar\) \{[\s\S]*?overflow-x: visible;/)
  assert.match(canvasCssSource, /@media \(max-width: 769px\)/)
  assert.match(pageHeaderSource, /\.header-inner \{[\s\S]*?min-width: 0;[\s\S]*?flex-wrap: wrap;/)
  assert.match(pageHeaderSource, /\.canvas-warning-bar \{[\s\S]*?max-width: 100%;[\s\S]*?flex-wrap: wrap;/)
  assert.match(pageHeaderSource, /overflow-x: clip;/)
  assert.match(pageHeaderSource, /@media \(max-width: 769px\)/)
  assert.match(toolbarSource, /role="group"/)
  assert.match(toolbarSource, /\.toolbar-main-row > \* \{[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;/)
  assert.match(toolbarSource, /\.workflow-group \{[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;/)
  assert.match(toolbarSource, /\.toolbar-utilities \{[\s\S]*?max-width: 100%;[\s\S]*?flex-wrap: wrap;/)
  assert.match(toolbarSource, /@media \(max-width: 1120px\)[\s\S]*?flex-wrap: wrap;/)
  assert.match(toolbarSource, /@media \(max-width: 769px\)[\s\S]*?flex-wrap: wrap;/)
  assert.doesNotMatch(toolbarSource, /\.canvas-desktop-toolbar \{[\s\S]*?overflow:\s*hidden/)
  assert.match(productionSidebarSource, /\.canvas-sidebar \{[\s\S]*?min-width: 0;[\s\S]*?max-width: 100%;/)
  ...
  assert.match(workspaceSource, /@media \(max-width: 769px\)[\s\S]*?overflow-x: visible;/)
})
