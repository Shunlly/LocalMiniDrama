...
test('制作页、项目列表、AI 配置和任务取消都接入本地操作日志', () => {
  ...
  const filmListLoadSource = read('../src/components/filmList/useFilmListLoad.js')
  ...
  assert.match(filmListSource, /useFilmListLoad\(/)
  assert.match(filmListLoadSource, /import \{ createOperationId, logOperation \} from '@\/utils\/operationLog'/)
  assert.match(filmListLoadSource, /operation: 'project_list_load'/)
  assert.match(filmListLoadSource, /phase: 'start'/)
  assert.match(filmListLoadSource, /phase: 'success'/)
  assert.match(filmListLoadSource, /phase: 'error'/)
  assert.match(filmListLoadSource, /phase: 'cancel'/)
...
test('core JSON 与成片下载失败都会留下带 requestId 和 category 的 http_request 日志', async () => {
  ...
  assert.doesNotMatch(coreSource, /operationId:\s*requestId/)
  assert.doesNotMatch(requestSource, /operationId:\s*requestId/)
  ...
  assert.match(String(logs[0].operationId || ''), /^http_request-/)
  assert.notEqual(logs[0].operationId, logs[0].details.requestId)
  assert.match(String(logs[0].details.requestId || ''), /^[A-Za-z0-9._:-]{1,128}$/)
  assert.match(logs[0].error, /^名称不能为空/)
  assert.match(logs[0].error, /请求编号：/)
  assert.doesNotMatch(JSON.stringify(logs), /无法连接服务/)
  assert.doesNotMatch(JSON.stringify(logs), /Select a valid model|Internal Server Error|Failed to fetch/i)
})
