...
test('project cards use a keyboard link while keeping the action menu outside it', () => {
  ...
  assert.match(filmListSource, /<\/RouterLink>[\s\S]*<RouterLink[\s\S]*class="project-card-assets"[\s\S]*<\/RouterLink>\s*<el-dropdown[\s\S]*class="project-card-menu"/)
...
test('desktop creation surfaces keep focused tasks readable and user-facing', () => {
  ...
  assert.match(canvasSource, /canvasMode\.value === 'free' \? selectedFreeNodeId\.value : focusedNodeId\.value/)
  assert.match(canvasSource, /openAiConfig\(freeCanvasConfigRuntimeById\.value\.get\(String\(nodeId\)\)\?\.serviceType \|\| 'video', nodeId\)/)
  ...
  assert.match(storyboardConfigBarSource, /帮助视频保持镜头衔接/)
  assert.match(storyboardConfigBarSource, /可直接用于长提示词的分段描述/)
...
test('completed source workflows hand off to production without obscuring the episode list', () => {
  ...
  assert.match(completionSummaryRule, /grid-template-columns:\s*minmax\(0, 0\.8fr\) minmax\(0, 1\.5fr\) auto/)
  assert.doesNotMatch(completionSummaryRule, /minmax\(180px/)
  assert.doesNotMatch(completionSummaryRule, /minmax\(360px/)
