...
test('film create navigation and resource disclosure controls use native buttons', () => {
  ...
  assert.match(surfaceBindingsSource, /onScrollToAnchor: scrollToAnchor/)
  assert.match(deliveryPanelSource, /<button[\s\S]*data-testid="delivery-empty-action"/)
  assert.doesNotMatch(deliveryPanelSource, /href="#anchor/)
...
test('制作页加载失败面保持可读状态和重试入口', () => {
  ...
  assert.match(projectLoadStateSource, /describeActionAriaLabel\('重试加载'/)
  assert.match(projectLoadStateSource, /正在重试加载/)
