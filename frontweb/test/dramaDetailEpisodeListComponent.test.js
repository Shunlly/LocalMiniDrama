...
test('没有剧集时展示中文空态，生成入口带禁用原因', async () => {
  ...
  try {
    ...
    assert.equal(empty.props['aria-label'], '空剧集下一步')
    assert.equal(buttonByAriaLabel(harness.root, '新增一集'), undefined)
    const primaryButtons = findAll(harness.root, (node) => node.type === 'button' && node.props?.['data-variant'] === 'primary')
    assert.equal(primaryButtons.length, 1)
    assert.match(textContent(primaryButtons[0]), /从素材生成剧集/)
...
test('主按钮已是新增空白集时不再重复渲染第二个新增空白集', async () => {
  ...
  try {
    ...
    const primaryButtons = findAll(harness.root, (node) => node.type === 'button' && node.props?.['data-variant'] === 'primary')
    assert.equal(primaryButtons.length, 0)
    assert.notEqual(addButtons[0].props['data-variant'], 'primary')
    click(addButtons[0])
    click(buttonByText(harness.root, '去导入素材'))
    click(buttonByText(harness.root, '批量导入剧本'))
    assert.deepEqual(harness.events, [
