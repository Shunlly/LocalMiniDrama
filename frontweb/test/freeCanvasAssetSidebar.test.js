...
test('素材栏空态区分筛选和真正没有素材，并给出下一步', () => {
  ...
  assert.match(componentSource, /清除素材筛选/)
  ...
  assert.match(componentSource, /aria-label="上传素材"/)
  assert.match(componentSource, /aria-label="从素材中心选择"/)
  assert.match(componentSource, /function isAssetSectionOpen/)
  assert.match(componentSource, /Number\(filteredCount\) === 0/)
  assert.match(componentSource, /aria-label="清除素材筛选"/)
  assert.match(componentSource, /aria-label="去制作页添加角色"/)
  assert.match(componentSource, /'go-production'/)
})
