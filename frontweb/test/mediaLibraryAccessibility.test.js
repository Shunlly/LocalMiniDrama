...
test('the initial empty state has one clearly named primary upload action', () => {
  ...
  assert.match(actions, /<el-button type="default" aria-label="去搜网络素材"/)
  assert.match(actions, /aria-label="上传素材"/)
  assert.doesNotMatch(actions, /<el-button[^>]*type="primary"[^>]*aria-label="去搜网络素材"/)
...
test('上传失败保留可见反馈，网络空结果不会伪装成成功列表', () => {
  ...
  assert.match(source, /aria-label="重新搜索"/)
  assert.match(source, /aria-label="清除搜索"/)
})

test('顶栏上传按钮与筛选空态都提供明确的上传名称', () => {
  assert.ok((source.match(/aria-label="上传图片或视频到素材中心"/g) || []).length >= 2)
  assert.ok((source.match(/aria-label="上传素材"/g) || []).length >= 2)
...
test('素材中心禁用按钮给出中文原因', () => {
  ...
  assert.match(source, /id="media-write-lock-reason"/)
  assert.match(source, /id="media-empty-upload-reason"/)
  assert.match(source, /id="media-network-search-reason"/)
  assert.match(source, /:aria-describedby="mediaRetryLoadDisableReason \? 'media-retry-load-reason' : 'media-list-load-error'"/)
  assert.match(source, /:aria-describedby="mediaBatchDeleteDisableReason \? 'media-batch-delete-reason' : undefined"/)
  assert.match(source, /:aria-describedby="networkSearchDisableReason \? 'media-network-search-reason' : undefined"/)
  assert.match(source, /:aria-describedby="mediaWriteLocked \? writeLockDescribedBy : undefined"/)
  assert.match(source, /:id="`network-import-reason-\$\{index\}`"/)
  assert.match(source, /:aria-describedby="\(isNetworkImporting\(item\) \|\| !networkItemImportability\(item\)\.allowed\) \? `network-import-reason-\$\{index\}` : undefined"/)
  ...
  assert.match(source, /class="network-state" role="status" aria-live="polite"/)
