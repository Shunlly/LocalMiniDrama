...
test('production E2E verifies workflow-first disclosures and AI config modes', () => {
  ...
  assert.match(productionSource, /readinessTitle: '\\u6210\\u7247\\u4ea4\\u4ed8\\u5c31\\u7eea\\u5ea6'/)
...
test('browser acceptance contract covers the full UI journey, recovery, downloads, viewports, and playback', () => {
  ...
  assert.match(productionSource, /new URL\(page\.url\(\)\)\.hash,[\s\S]*#episode-list/)
  assert.match(productionSource, /importSourceFromUi[\s\S]*#source-intake-workflow/)
...
test('backend container gives Node PID 1 and uses a stable maintenance lease scope', () => {
  ...
  assert.match(productionDockerfile, /COPY frontweb\/nginx\.conf \/etc\/nginx\/conf\.d\/default\.conf/)
  const healthzStart = productionNginxConfig.indexOf('location = /healthz {')
  const readyStart = productionNginxConfig.indexOf('location = /ready {')
  const spaStart = productionNginxConfig.indexOf('location / {')
  ...
  const healthzBlock = productionNginxConfig.slice(healthzStart, productionNginxConfig.indexOf('}', healthzStart) + 1)
  const readyBlock = productionNginxConfig.slice(readyStart, productionNginxConfig.indexOf('}', readyStart) + 1)
