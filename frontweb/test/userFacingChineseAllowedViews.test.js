...
test('SD2 资产库对话框、表单和反馈文案改为简体中文，接口名保持原文', () => {
  ...
  assert.match(sd2Source, /call\('ListAssetGroups'/)
  ...
  assert.match(sd2Files.connectionForm, /若仍提示没有权限/)
  assert.doesNotMatch(sd2Files.connectionForm, /Invalid Authorization/)
  assert.doesNotMatch(sd2Files.connectionForm, /报\s*40[13]/)
  assert.doesNotMatch(sd2Files.connectionForm, /\b(?:HTTP\s*)?40[13]\b/)
  const sd2VisibleCopy = [sd2Files.intro, sd2Files.connectionForm, sd2Files.groupList, sd2Files.assetList, sd2Files.filter, sd2Files.dialogs, sd2Files.lastResponse].join('\n')
  assert.doesNotMatch(sd2VisibleCopy, /Invalid Authorization/)
  assert.doesNotMatch(sd2VisibleCopy, /ListAssetGroups/)
  assert.doesNotMatch(sd2VisibleCopy, /\bHTTP\s*\d{3}\b/)
