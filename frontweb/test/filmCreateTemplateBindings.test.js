...
const shellBindingsSource = read('../src/components/filmCreate/filmCreateShellBindings.js')
...
test('制作页把素材警告、交付区和弹窗层交给独立组件', () => {
  ...
  assert.match(shellBindingsSource, /mediaError: storyboardMediaLoadError/)
  assert.match(shellBindingsSource, /onRetry: retryProjectDependencies/)
  ...
  assert.match(shellBindingsSource, /resourceDialogs: resourceDialogsBindings/)
  assert.match(shellBindingsSource, /storyboardDialogs: storyboardDialogsBindings/)
  assert.match(shellBindingsSource, /maxChapters: novelMaxChapters/)
  assert.match(shellBindingsSource, /modelValue: showAiConfigDialog/)
