...
test('production control names preserve generation and item context', () => {
  ...
  assert.match(controlBy(storyboardConfigBarSource, 'el-input-number', 'v-model="storyboardCount"'), /aria-label="分镜数量（生成设置）"/)
  assert.match(controlBy(novelImportSource, 'el-input-number', 'v-model="maxChapters"'), /aria-label="最多导入集数"/)
