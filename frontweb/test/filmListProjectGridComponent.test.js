...
test('空列表不渲染卡片；无封面时展示中文空态', async () => {
  ...
  try {
    ...
    const stats = findByClass(harness.root, 'project-card-stats')
    assert.equal(stats[0].props['aria-label'], '项目「未命名项目」概览')
    assert.equal(stats[1].props['aria-label'], '项目「雨巷」概览')
