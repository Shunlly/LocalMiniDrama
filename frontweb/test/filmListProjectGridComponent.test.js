...
test('导入意图下卡片改成导入网页 URL，菜单命令交给页面', async () => {
  ...
  try {
    ...
    const assets = linkByAriaLabel(harness.root, '打开项目「雨巷」的故事素材流程')
    assert.ok(assets)
    assert.equal(JSON.parse(assets.props['data-to']).name, 'drama-detail')
    assert.equal(JSON.parse(assets.props['data-to']).hash, '#source-intake-workflow')
