...
test('有剧集时空态提供生成和添加分镜，点击交给页面方法', async () => {
  ...
  try {
    ...
    const primaries = findByType(harness.root, 'button').filter((node) => node.props['data-variant'] === 'primary')
    assert.equal(primaries.length, 1)
    assert.match(textContent(primaries[0]), /生成分镜/)
    assert.match(String(primaries[0].props['aria-label'] || ''), /生成分镜/)
...
test('没有剧集时只提供去创建剧集，不展示生成分镜', async () => {
  ...
  try {
    ...
    assert.notEqual(create.props['data-variant'], 'primary')
    click(create)
