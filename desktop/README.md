...
基于 Electron 的本地桌面应用，内嵌 `backend-node` 与 `frontweb`。当前包版本为 **1.3.3 发布候选**，候选目标为 Windows x64 Setup 与 Portable；Git 尚无 `v1.3.3` 标签或正式 Release，本地构建产物不是 GitHub 正式发布。不要把历史 SHA 或当前工作树当作已通过 Docker/浏览器验收；容器级校验用仓库根目录 `npm run verify:docker`，生产 E2E 必须在干净工作树按 `npm run docker:e2e:up` → `npm run verify:e2e` 重跑。正式二进制必须等同一 Git SHA 的全部门禁通过并经人工发布。macOS 构建会主动拒绝执行，不在当前验收矩阵。桌面依赖安装、原生重建、打包和 Windows 制品安全扫描固定使用 Node.js 22.12.0。
...
自由画布当前只验收桌面键鼠范围。移动/触控、新真实 Provider 路由、协作与完整 Agent/MCP 后置；自动化测试不调用外部真实 Provider。2026-07-27 收尾报告位于 `http://127.0.0.1:3013/reports/infinite-canvas-20260727/report.html`，只覆盖该日范围，不能替代当前工作树重跑 `npm run verify:docker` / `npm run verify:e2e`。
