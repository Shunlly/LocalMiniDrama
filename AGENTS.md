...
- The backend also serves the built frontend from `frontweb/dist/` at port 5679 when the dist folder exists (`WEB_DIST_PATH` can override the path); during development, use the Vite dev server at port 3013 instead.
...
- Production Nginx (`frontweb/nginx.conf`) must proxy `location = /ready` to backend `/ready` before the SPA `location /` fallback. `/healthz` alone is not enough because the backup page requests `/ready`.
